...
RUN dotnet build -c Release --no-restore src/AM180/AM180.csproj

FROM dotnet-build AS dotnet-test
RUN dotnet test -c Release --no-build --no-restore src/AM180/AM180.csproj

FROM dotnet-build AS publish
RUN dotnet publish -c Release --no-build --no-restore -o /app src/AM180/AM180.csproj
